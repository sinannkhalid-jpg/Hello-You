          ...
          <Card>
            <CardContent className="p-0">
              <ul className="divide-y divide-white/5">
                {data.map((i: any) => (
                  <li key={i.id} className="p-4 flex items-center gap-3 hover:bg-white/5 transition-colors">
                    ...
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" onClick={() => favM.mutate(i.id)} aria-label="Favorite">
                        <Star className={`h-4 w-4 ${i.is_favorite ? "fill-yellow-300 text-yellow-300" : "text-muted-foreground"}`} />
                      </Button>
                      <DropdownMenu>
